provider "azurerm" {
  features {}
#new code added to read variables from .tfvars
...
resource "null_resource" "enable-features" {
  provisioner "local-exec" {
    interpreter = ["/bin/bash" ,"-c"]
    command = <<EOT
      az login --service-principal -t ${var.tenant_id} -u ${var.client_id} -p ${var.client_secret}
...
resource "null_resource" "preview-features" {
  provisioner "local-exec" {
    interpreter = ["/bin/bash" ,"-c"]
    command = <<EOT
  ...
  depends_on = [null_resource.enable-features]
}
...
resource "null_resource" "add-agic" {
  ...
  depends_on = [azurerm_application_gateway.network, null_resource.attach-acr, null_resource.enable-features]
}
...
resource "null_resource" "agic-identity" {
  ...
  depends_on = [azurerm_resource_group.rg, azurerm_application_gateway.network, azurerm_kubernetes_cluster.aks, null_resource.add-agic, null_resource.enable-features]
}